...
namespace MapleStoryHelper.Framework.ResourceManager
{
    public partial class MapleWz
    {
        ...
        public List<MobBase> GetMobs(string keyWord, EMobMapCategory category)
        {
            List<MobBase> retval = new List<MobBase>();
        ...
        private List<MobBase> GetMobsFromNodes(List<Wz_Node> nodes)
        {
            ...
            for(int i = 0; i < nodes.Count; i++)
            {
                ...
                if(mob != null)
                {
                    ...
                    if(newItem.Default.Bitmap != null)
                    {
                        MemoryStream stream = new MemoryStream();
                        newItem.Default.Bitmap.Save(stream, ImageFormat.Png);
                        newItem.ImgByte = stream.ToArray();
                    }
